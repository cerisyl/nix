...
if pkgMap ? "tauon" then {
  ...
  home.activation.tauonConfig = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    if [ ! -f "$config" ]; then
      mkdir -p "${homedir}/.local/share/TauonMusicBox"
    fi
    cat > "$config" << EOF
      [audio]
      ...
      avoid_resampling = false        # Only implemented for FLAC, MP3, OGG, OPUS
      ...
      tag-editor-target = ""  # The path of the exe to run.
      ...
      font-main-standard        = "Barlow, Noto Sans, Noto Sans CJK JP, Arial,"
      font-main-medium          = "Barlow, Noto Sans, Noto Sans CJK JP, Arial, Medium"
      font-main-bold            = "Barlow, Noto Sans, Noto Sans CJK JP, Bold"
      font-main-condensed       = "Barlow, Noto Sans, Extra-Condensed"
      font-main-condensed-bold  = "Barlow, Noto Sans, Extra-Condensed Bold"
      ...
      [app]
      enable-remote-interface = false  # For use with Tauon Music Remote for Android
      ...
      auto-dl-artist-data = false     # Enable automatic downloading of thumbnails in artist list
